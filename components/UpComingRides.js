...
const UpComingRides = () => {
	...
	useEffect(() => {
		setRides([])
		db.collection('rides').where('driverID', '==', auth.currentUser.uid).where("status", "==", "upcoming")
			.get()
			.then((querySnapshot) => {
					...
					db.collection("passengers").doc(rideDetails?.userID).get()
						.then((doc) => {
							if (doc.exists) {
								...
								setRides((p) => [...p, {
									passengerName: (doc.data().firstName + " " + doc.data().lastName) ?? "Not Provided",
									...rideDetails
								}])
							} else {
		...
		>
			{
				rides?.length > 0 && rides != null 
					? <FlatList
						data={rides}
						keyExtractor={(_, index) => `upcoming-ride-${index}`}
										...
										>
											...
											>
												<View style={{ alignItems: 'center' }}>
													<Text style={{ ...FONTS.h3 }}>{item?.passengerName}</Text>
													<StarReview rate={4.5} />
												</View>
												<View style={{ alignItems: 'center' }}>
					...
					>
						...
						<Text style={{ marginTop: SIZES.radius, ...FONTS.h3, fontSize: 18 }}>
							You don't have any coming rides
						</Text>
					</View>
			}
		</View>
